...
# Define custom labels with units for each parameter
custom_labels <- as_labeller(c(
  "ER" = "ER (mg O2 L^-1 d^-1)",
  "NEP" = "NEP (mg O2 L^-1 d^-1)",
  "GPP" = "GPP (mg O2 L^-1 d^-1)",
  "K" = "K (d^-1)",
  "DO" = "DO (mg L^-1)"
))

# Plot with facet_wrap and conditional y-axis limits
p <- ggplot(DataLong, aes(x = Season, y = Value, fill = Site, pattern = Site)) +
  ...
  # Facet with custom labels and free y-scales
  facet_wrap(~Parameter, scales = "free_y", ncol = 1, labeller = custom_labels) +
  labs(y = "Value") +
...
# Add a y-limit column to filter values conditionally for ER and NEP
DataLong <- DataLong %>%
...
# plot out k vs NEP
```{r}
p <- ggplot(Data, aes(x = K.mean, y = NEP.mean, color = Season, shape = Site)) +
  geom_point() +
  #geom_line() +
  scale_color_manual(values=c("Winter"="#4DADCF", "Summer"= "coral1","Fall"="#E9943A", "Spring"="#5EC795")) +
