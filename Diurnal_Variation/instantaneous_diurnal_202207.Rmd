...
# Updated plot with facet label for KO2
p <- ggplot(DataFacetLong, aes(x = DateTime)) +
  ...
  facet_wrap(~variable, scales = "free", nrow = 2) +  # Apply the custom labeller
  labs(x = "DateTime",  # Adjust x-axis label
       y = "Value",     # Adjust y-axis label
       color = "Log(PAR)") +  # Adjust legend label
  theme(axis.text = element_text(size = 10),      # Adjust font size of axis text
...
corrplot(Corr, method="circle", type = 'lower', order = "hclust", addCoef.col = 'black',
         p.mat = testRes, insig='blank', tl.col = 'black', tl.cex = 2,
...
```
```{r}
# Define the cor.mtest function to calculate the p-values
